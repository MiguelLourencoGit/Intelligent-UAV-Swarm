...
def path_loss(d, d0=1, L_d0=30, n=2.7):
    ...
    if d <= 0:
        d = 1e-6  # Valor muito pequeno em vez de zero
    if d0 <= 0:
        d0 = 1
...
def calcular_capacidade_link(
    ...
    # Evitar distância zero (UAVs na mesma posição)
    if d <= 0:
        d = 1e-6  # Valor muito pequeno em vez de zero
...
def interference_from_jammer(P_jammer_dBm, G_jammer_i, lambda_0, d_jammer_i):
    # Evitar distância zero
    if d_jammer_i <= 0:
        d_jammer_i = 1e-6  # Valor muito pequeno em vez de zero
